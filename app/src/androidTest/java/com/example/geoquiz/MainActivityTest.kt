...
class MainActivityTest {
    ...
    fun verifiesCurrentQuestionAnswer() {
        scenario.onActivity { activity ->
            assertTrue(activity.quizViewModel.currentQuestionAnswer) // First assertion
        }

        onView(withId(R.id.next_button)).perform(click())

        scenario.onActivity { activity ->
            assertTrue(activity.quizViewModel.currentQuestionAnswer) // Second assertion
        }

        onView(withId(R.id.next_button)).perform(click())

        scenario.onActivity { activity ->
            assertFalse(activity.quizViewModel.currentQuestionAnswer) // Third assertion
        }
    }
}